...
const App = () => {
  ...
  // GitHubの public フォルダに配置された画像パスに更新
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {['About', 'Studios', 'Database', 'Reports', 'Contact'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold hover:text-blue-600 transition-colors">
                {item === 'About' ? '協会について' : 
                 item === 'Studios' ? 'スタジオ' : 
                 item === 'Database' ? 'スタートアップ' : 
                 item === 'Reports' ? 'レポート' : 'お問い合わせ'}
              </a>
            ))}
            <button className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 transition-all shadow-md shadow-blue-500/20">
              加盟・参画
            </button>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-7xl font-black mb-8 leading-[1.15] tracking-tight">
              スタートアップ創出を<br />
              <span className="text-blue-400">なめらかに</span>
            </h1>
            <p className="text-lg md:text-xl text-gray-300 mb-12 leading-relaxed font-medium">
              日本におけるスタートアップスタジオの普及と、<br />
              次世代の起業家を支援するエコシステムの構築を目指します。
            </p>
            <div className="flex flex-wrap gap-4">
              <a href="#studios" className="bg-blue-600 text-white px-8 py-4 rounded-xl font-bold hover:bg-blue-700 transition-all flex items-center gap-2 shadow-lg shadow-blue-600/30">
                加盟スタジオを探す <ArrowRight size={20} />
              </a>
            </div>
          </div>
        </div>
      ...
      {/* 3. Featured Studios Section */}
      <section id="studios" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16">
            <h2 className="text-4xl font-black mb-4 flex items-center gap-3">
              <Rocket className="text-blue-600" /> Featured Studios
            </h2>
            <p className="text-slate-500 text-lg">日本を代表する主要なスタートアップスタジオ</p>
          </div>
          ...
          <div className="bg-slate-50 rounded-[2.5rem] p-10 md:p-16 border border-slate-100">
            <div className="flex items-center gap-4 mb-10">
              ...
              <h3 className="text-2xl font-black">Member Directory</h3>
              <div className="h-px flex-grow bg-slate-200 ml-4"></div>
              <span className="text-slate-400 font-bold text-sm">加盟スタジオ一覧</span>
            </div>
            ...
            <div className="mt-16 text-center">
              <button className="text-blue-600 font-bold flex items-center gap-2 mx-auto hover:gap-3 transition-all">
                すべての加盟スタジオを表示する <ChevronRight size={20} />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
            <div className="text-center md:text-left">
              ...
              <p className="text-gray-400 font-medium">スタジオから創出された事業・企業データベース</p>
            </div>
      ...
      <section id="reports" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 font-medium">スタートアップスタジオ協会の調査レポート</p>
          </div>
            ...
            {reports.map(report => (
              <div key={report.id} className="flex flex-col p-8 rounded-3xl border border-slate-200 hover:border-blue-500 transition-all bg-white group">
                ...
                <button className="mt-auto flex items-center justify-center gap-2 w-full py-3 bg-slate-900 text-white rounded-xl font-bold group-hover:bg-blue-600 transition-all">
                  資料をダウンロード <ChevronRight size={18} />
                </button>
              </div>
            ))}
          </div>
      ...
      <footer className="py-16 border-t border-slate-200 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
            <div className="flex items-center gap-2">
              <img src={logoUrl} alt="Logo" className="h-10 grayscale" />
            </div>
